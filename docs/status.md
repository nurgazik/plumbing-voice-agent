...
- Product decision for Ray: the tier is set at triage from the caller's words and never revised. On the 2026-09-15 call the caller could not shut the water off and the photo showed a supply-line leak, which the rules mark urgent, but the agent had already said routine and passed routine to send_quote. Should the photo's job type raise the tier (routine to urgent only, never lower)? If yes: a prompt rule plus an eval case, and the quote text is unchanged.
